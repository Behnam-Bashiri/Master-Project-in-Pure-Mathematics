...
########################
landa_x = sp.Symbol("x")
landa_x_2n = sp.Symbol("2x")
landa_x_2np1 = sp.Symbol("2xp1")
...
def v(n, j):
    if n < 8:
        ...
        #########
        sq1 = ((s_pi_TWOn / s_pi) * s_lanada_TWOn * X(TWOn))
        sq2 = ((s_pi_TWOnP1 / s_pi) * s_lanada_TWOnP1 * X(TWOnP1))
...
def v_X(n, j):
    ...
    s_lanada_TWOnP1 = data_res[selector_landa_TWOnP1]
    #########
    sq1 = ((s_pi_TWOn / s_pi) * s_lanada_TWOn * landa_x_2n)
    sq2 = ((s_pi_TWOnP1 / s_pi) * s_lanada_TWOnP1 * landa_x_2np1)
    selector_ro = "ro" + "{}".format(j)
    res_ro = data[selector_ro]
    return res_ro * (sq1 + sq2)
...
def Hemiltoni(n, j):
    if 7 < n:
        ...
        # res_landa_find2 = sp.diff(result, landa_x_2n)
        # res_landa_find3 = sp.diff(result, landa_x_2np1)
        # res_landa_find = res_landa_find1+res_landa_find2+res_landa_find3
        result = result.subs({landa_x: X(n)})
        result = result.subs({landa_x_2n: X(2*n)})
        result = result.subs({landa_x_2np1: X(2*n+1)})
        res_landa_find = res_landa_find.subs(landa_x, X(n))
        res_landa_find = res_landa_find.subs(landa_x_2n, X(2*n))
        res_landa_find = res_landa_find.subs(landa_x_2np1, X(2*n+1))
...
def calcuator():
    H1_1 = data_res["Hamiltoni1_1"]
    H2_1 = data_res["Hamiltoni2_1"]
    H3_1 = data_res["Hamiltoni3_1"]
    U1_1 = data["u1_1"]
    U2_1 = data["u2_1"]
    U3_1 = data["u3_1"]

    res1_2 = sp.diff(H1_1, U1_1)
    res2_2 = sp.diff(H2_1, U2_1)
    res3_2 = sp.diff(H3_1, U3_1)
